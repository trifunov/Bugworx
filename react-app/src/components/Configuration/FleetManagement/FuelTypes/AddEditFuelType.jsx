...
const AddEditFuelType = ({ isOpen, formData, isSaving, onUpdateFieldHandle, onClose, onSave }) => {
    ...
    <>
      <div className={`offcanvas offcanvas-end show`} style={{ width: '400px' }} tabIndex='-1'>
        <div className='offcanvas-header'>
          <h5 className='offcanvas-title'>{isEditing ? 'Edit Fuel Type' : 'Add New Fuel Type'}</h5>
          <button type='button' className='btn-close text-reset' onClick={onClose} aria-label='Close' disabled={isSaving}></button>
        </div>
        <div className='offcanvas-body'>
          <form onSubmit={handleSubmit}>
            <div className='mb-3'>
              <label className='form-label'>Fuel Type Name</label>
              <input name='name' className='form-control' required value={formData.name || ''} onChange={handleChange} placeholder='e.g., Gasoline' />
            </div>
            <div className='mb-3'>
            ...
            <div className='form-check form-switch mb-3'>
              <input
                ...
                id='fuel-type-active-check'
                name='active'
                checked={formData.active === undefined ? true : formData.active}
                onChange={handleChange}
              />
              <label className='form-check-label' htmlFor='fuel-type-active-check'>
                Active
              </label>
            </div>
