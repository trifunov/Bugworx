...
const AddEditInventory = ({ isOpen, formData, errors, isSaving, onUpdateFieldHandle, onClose, onSave }) => {
    ...
    return (
        <>
            <div className={`offcanvas  offcanvas-end ${isMounted ? 'show' : ''}`} style={{ '--bs-offcanvas-width': '45%' }} tabIndex="-1">
                        ...
                        <div>
                            ...
                            <AccordionItem id="stock" title="Stock & Units" className="custom-accordion" isOpen={openAccordions.stock} onToggle={toggleAccordion}>
                                <div className="row g-3">
                                    ...
                                    <div className="col-md-6"><label htmlFor="cost" className="form-label">Cost per Unit</label><input id="cost" name="cost" className={`form-control ${errors.cost ? 'is-invalid' : ''}`} type="number" value={formData.cost} onChange={handleFieldChange} /></div>
                                    <div className="col-md-6"><label htmlFor="price" className="form-label">Selling Price per Unit</label><input id="price" name="price" className={`form-control ${errors.price ? 'is-invalid' : ''}`} type="number" value={formData.price} onChange={handleFieldChange} /></div>
