  ...
  emergencies.forEach(emergency => {
    ...
    if (hoursDifference > 2) {
      alerts.push({
        ...
        message: `🚨 EMERGENCY at ${emergency.serviceAddressName || 'Unknown Service Address'} scheduled for ${emergency.scheduledTime} (${hoursDifference.toFixed(1)}h from now)`,
    ...
    if (!emergency.technicianId) {
      alerts.push({
        ...
        severity: 'critical',
        appointmentId: emergency.id,
        message: `⚠️ CRITICAL: Emergency at ${emergency.serviceAddressName || 'Unknown Service Address'} has no technician assigned`,
