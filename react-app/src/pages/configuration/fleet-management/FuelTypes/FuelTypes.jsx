...
import AddEditFuelType from '../../../../components/Configuration/FleetManagement/FuelTypes/AddEditFuelType';
import { useAddEditFuelType } from '../../../../components/Configuration/FleetManagement/FuelTypes/useAddEditFuelType';
...
const FuelTypes = () => {
  const { items, saveItem, removeItem } = useFuelTypes();
  const addEditFuelType = useAddEditFuelType(saveItem);
  const { setPageSubHeader } = usePageSubHeader();

  useEffect(() => {
    setPageSubHeader({
      title: 'Fuel Type & Capacity',
      breadcrumbs: [
        { label: 'Configuration', path: '/configuration' },
        { label: 'Fleet Management', path: '/configuration/fleet-management' },
        { label: 'Fuel Types', active: true },
      ],
    });
  }, [setPageSubHeader]);
  ...
  return (
    <>
      <AddEditFuelType
        isOpen={addEditFuelType.isOpen}
        formData={addEditFuelType.formData}
        isSaving={addEditFuelType.isSaving}
        onUpdateFieldHandle={addEditFuelType.onUpdateFieldHandle}
        onClose={addEditFuelType.close}
        onSave={addEditFuelType.onSaveHandle}
      />
                ...
                <div className='col-12'>
                  <div className='d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-2'>
                    ...
                    <div className='mt-2 mt-md-0'>
                      <AddNewButton handleAddNew={() => addEditFuelType.open()} />
                    </div>
                  </div>
                </div>
              </div>
