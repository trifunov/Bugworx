...
const FrequencyTemplates = () => {
  const { items, saveItem, removeItem } = useFrequencyTemplates();
  const { addEditFrequencyTemplate } = useEditableFormContext();
  ...
  const { filteredItems, searchTerm, setSearchTerm } = useTableSearch(items, ['name', 'repeatSchedule']);
  const { data: paginatedData, ...tableProps } = useTable(filteredItems, { defaultSortField: 'name' });

  const renderRow = (item) => (
    <tr key={item.id}>
      <td><strong>{item.name}</strong></td>
      <td>{item.repeatSchedule}</td>
      <td>{item.description}</td>
      <td>
        <div className="d-flex gap-3">
          <a className="text-primary" href="#" title="Edit" onClick={(e) => { e.preventDefault(); addEditFrequencyTemplate.open(item); }}><i className="mdi mdi-pencil font-size-18"></i></a>
  ...
  return (
    <>
      <AddEditFrequencyTemplate
        isOpen={addEditFrequencyTemplate.isOpen}
        formData={addEditFrequencyTemplate.formData}
        isSaving={addEditFrequencyTemplate.isSaving}
        onUpdateFieldHandle={addEditFrequencyTemplate.onUpdateFieldHandle}
        onClose={addEditFrequencyTemplate.close}
        onSave={() => addEditFrequencyTemplate.onSaveHandle(saveItem)}
      />
            ...
            <div className="card-body">
              ...
              <Table
                ...
                emptyState={{
                  ...
                  message: 'No frequency templates found. Click "Add New" to create one.'
                }}
              />
            </div>
          </div>
